...
void ParseCommand(int ComPort)
{
    ...
    switch (RxBuffer[0][ComPort]) 
    {
        case 'A':
            ...
            Target1=Target1-OffsetAdjust;      // Compensate for iRacing Heave offset until fixed in Simtools plugin.
        ...
        case 'B':
            ...
            Target2=Target2-OffsetAdjust;      // Compensate for iRacing Heave offset until fixed in Simtools plugin.
#ifdef ENABLE_POT_SCALING
            if (PotInput < 25)
            {
...
//                    default:
...
#endif
            break;
        case '-':             // used as an offset adjustment for iRacing until it is fixed in Simtools plugin
            OffsetAdjust=(RxBuffer[1][ComPort]*256)+RxBuffer[2][ComPort];
            OffsetAdjust=OffsetAdjust-511;
            break;
        case 'm':
        ...
        case 'v':
            ...
            {
                SendValue('v',80,ComPort);     // Software Version - divide by 100 to get version - ie 101= ver1.01
            }
            break;
        case 'e':
...
#ifdef MODE3    // Sabretooth packet serial mode

//****************************************************************************************************************
//    Software Serial is used to command motor speed and direction for two motors
//          
//****************************************************************************************************************
...
//    {
//        PWMout3 = PWMrev3;
//        ST[1].motor(1,(constrain((PWMout3/2),-127,127)));    
//    }
//    else if((Feedback3<InputClipMin3) && (PWMrev3 != 0))
//    {
//        PWMout3 = PWMrev3;
//        ST[1].motor(1,(constrain(0-(PWMout3/2),-127,127)));    
//    }  
//    else if(Target3 > (Feedback3 + DeadZone3) || Target3 < (Feedback3 - DeadZone3))
//    {
//        if (PWMout3 >= 0)  
...
int CalcMotor1PID(int TargetPosition, int CurrentPosition)   
{
    ...
    pTerm_x100 = (Kp1_x100 + Kp1_x100) * (long)Error;                    // Error can only be between +/-1023 and Kp1_100 is constrained to 0-1000 so can work with type long
...
int CalcMotor2PID(int TargetPosition, int CurrentPosition)   
{
    ...
    pTerm_x100 = (Kp2_x100 + Kp2_x100) * (long)Error;                    // Error can only be between +/-1023 and Kp1_100 is constrained to 0-1000 so can work with type long
...
int CalcMotor3PID(int TargetPosition, int CurrentPosition)   
{
    ...
    pTerm_x100 = (Kp3_x100 + Kp3_x100) * (long)Error;                  // Error can only be between +/-1023 and Kp1_100 is constrained to 0-1000 so can work with type long
...
//   Serial.write(OCR1AH);
   ...
   mySerial.write('o');
    ...
    if(softstart != 256)
    {
     ...
     if(counter > SSloop)
     {
      if(PWMmax1 < holdmax1) {PWMmax1++;}
      if(PWMmax2 < holdmax2) {PWMmax2++;}
      if(PWMmax3 < holdmax3) {PWMmax3++;}
      softstart++;
      counter = 0;
     }
  }
